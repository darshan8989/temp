...
1. **Vercel**: import the repo, then deploy with `npm run build` and output folder `dist`.
2. **Netlify**: import the repo, then deploy with build command `npm run build` and publish folder `dist`.

This repo already includes `vercel.json` and `netlify.toml` for SPA routing.
